...
const defaultSourcePath = path.join(root, "data", "bangyan-direct-bathrobe.md");
const sourcePath = process.argv[2] ? path.resolve(process.argv[2]) : defaultSourcePath;
const category = "原图处理";
const subcategory = "浴巾浴袍";
...
if (parsed.length !== expectedCount) throw new Error(`浴巾浴袍 Prompt 应为 ${expectedCount} 条，实际解析到 ${parsed.length}`);
if (parsed.some((entry) => entry.group !== subcategory)) throw new Error("浴巾浴袍来源文件包含未配置分组");
...
for (const [index, entry] of parsed.entries()) {
  ...
  const id = existingEntry?.id || `direct_bathrobe_${String(index + 1).padStart(2, "0")}`;
...
console.log(`浴巾浴袍 Prompt 导入完成：新增 ${additions.length} 条，覆盖 ${replacements.size} 条，原样保留 ${unchanged.length} 条；总直接 Prompt ${directPrompts.length} 条。`);
